  ...
  const columns: Column<WorkOrderListItem>[] = [
    {
      key: 'n',
      header: 'OT',
      cell: (w) => <span className="code">{w.number}</span>,
      className: 'w-[150px]',
    },
    {
      key: 'c',
      ...
      cell: (w) => (
        <div className="min-w-0 max-w-[360px]">
          ...
          <p className="truncate text-xs text-text-muted">
            {w.equipment ? `${w.equipment.code} · ${w.equipment.name}` : w.title}
          </p>
        </div>
      ),
    },
    {
      key: 'sv',
      header: 'Servicio',
      cell: (w) => <span className="line-clamp-1 max-w-[220px]">{w.serviceType.name}</span>,
      hideBelow: 'xl',
    },
    {
      key: 't',
      header: 'Técnico',
      cell: (w) => w.technician?.fullName ?? <span className="text-text-muted">Sin asignar</span>,
      hideBelow: 'lg',
    },
    {
      key: 'd',
      header: 'Programada',
      cell: (w) => <span className="tabular whitespace-nowrap">{fmtShort(w.scheduledStart)}</span>,
      hideBelow: 'lg',
    },
    {
      key: 'p',
      ...
      <Panel flush>
        <div className="grid gap-2 border-b border-border p-3 md:grid-cols-[1fr_190px_190px] xl:grid-cols-[1fr_190px_190px_150px_170px]">
          <SearchInput
            ...
            placeholder="OT-2026-000012, cliente, serial, técnico…"
          />
          <Select
            aria-label="Estado"
          ...
          >
            ...
            {statusFilterValue === 'multi' && (
              <option value="multi">Selección del dashboard</option>
            )}
        ...
        <DataTable
          ...
          onRowClick={(w) =>
            navigate(
              w.status === WorkOrderStatus.PENDING_REVIEW
                ? `/work-orders/${w.id}/review`
                : `/work-orders/${w.id}`,
            )
          }
          mobileCard={(w) => (
          ...
          empty={
            <EmptyState
              ...
              description="Cambie los filtros o cree una nueva orden de trabajo."
            />
          }
        />
